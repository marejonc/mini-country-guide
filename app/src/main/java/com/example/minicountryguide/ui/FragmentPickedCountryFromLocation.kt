        ...
        if (!Utilities.checkLocationPermission(requireContext())) { return }
            ...
            fusedLocationClient.lastLocation.addOnSuccessListener { location: Location? ->
                ...
                viewModel.countryData.observe(viewLifecycleOwner) {
                    ...
                    val neighbours =
                        Utilities.printNeighbours(viewModel.countryData.value?.borders!!)
                    val area = viewModel.countryData.value?.area!!
                    val population = viewModel.countryData.value?.population!!
                    val timeZones =
                        Utilities.printListedData(viewModel.countryData.value?.timezones!!)
                    val carSign = viewModel.countryData.value?.car!!.signs[0]
                    val carSide =
                        Dictionaries.findPolishSide(viewModel.countryData.value?.car!!.side)

                    val dataList = arrayListOf(
                        listOf(
                            resources.getString(R.string.official_polish_name_caption),
                            officialName
                        ),
                        listOf(
                            resources.getString(R.string.common_english_name_caption),
                            commonEnglishName
                        ),
                        listOf(
                            resources.getString(R.string.official_english_name_caption),
                            officialEnglishName
                        ),
